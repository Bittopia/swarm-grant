...
export const actions = {
	newQuestionnarie: async ({ locals, request }: RequestEvent) => {
		const [, societyId, , courseId, , moduleId] =
			ObjectPathResolverUtil.getObjectPathFromUrl(request.url) as string[];
		...
		const questionnarie: NewQuestionType = {
			...
			creator: locals.user.web3Address,
		};
